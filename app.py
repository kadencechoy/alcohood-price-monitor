...
st.caption(
    "Google Ads Search Terms • Browser Agent Search • AI Product Extraction • Pricing Action"
)
...
def load_google_ads_csv(uploaded_file):
    ...
    clean_df["Ad Cost"] = (
        df[cost_col].apply(clean_number)
        if cost_col
        else 0
    )
    ...
    clean_df["Demand Score"] = (
        clean_df["Clicks"] * 3
        + clean_df["Impressions"] * 0.01
        + clean_df["Ad Cost"] * 0.05
    )
...
Important:
Google Ads Cost is advertising spend only. It is NOT retail price.
...
Find the best matching product and retail price from this rendered website text.
...
Rules:
- Only return a result if it is a real alcohol product listing.
- Do NOT use delivery fee, shipping fee, membership fee, event fee, deposit, coupon value, or Google Ads cost as product price.
...
Alcohood:
{alcohood_name}
Retail Price: HK${alcohood_price}

Competitor:
{competitor_name}
{competitor_product}
Retail Price: HK${competitor_price}

Rule:
If same SKU and Alcohood is more expensive, suggested price = competitor retail price - 1.
        ...
        rows.append(
            {
                ...
                "Ad Cost": row.get("Ad Cost", 0),
                "Demand Score": row.get("Demand Score", 0),
                "AI Type": ai.get("type"),
...
def build_browser_price_radar(product_df, selected_competitors):
    ...
    for index, (_, row) in enumerate(product_rows.iterrows(), start=1):
        ...
        status_box.info(f"Searching {index}/{total}: {product_query} on Alcohood")
        ...
        main_rows.append(
            {
                ...
                "Alcohood Retail Price": alcohood_price,
                ...
                "Competitor Retail Price": competitor_price,
                "Competitor URL": competitor_url,
                "Retail Price Difference": difference,
                "Suggested Retail Price": suggested_price,
                "Status": status,
                ...
                "AI Suggested Retail Price": ai_match.get("ai_suggested_price"),
            }
        )
...
st.info(
    "This version uses Google Ads Search Terms for market demand. "
    "Ad Cost is advertising spend only, not product retail price. "
    "Retail prices are extracted from Alcohood and competitor websites."
)
...
if uploaded_file:
    ...
    if not ads_df.empty:
        ...
        if run_price_radar and "classified_df" not in st.session_state:
            st.warning("No AI classification found. I will run classification first.")

            with st.spinner("AI is classifying Google Ads search terms first..."):
                classified_df = classify_ads_terms(
                    ads_df,
                    max_terms,
                )
            ...
            st.session_state["classified_df"] = classified_df
            st.success("✅ AI classification completed automatically.")
        ...
        if run_price_radar:
            ...
            classified_df = st.session_state.get(
                "classified_df",
                pd.DataFrame(),
            )

            if classified_df.empty:
                st.error("❌ AI classification is empty. Please check OpenAI classification.")
                st.stop()
        ...
        if "radar_df" in st.session_state:
            ...
            detail_df = st.session_state.get(
                "detail_df",
                pd.DataFrame(),
            )
